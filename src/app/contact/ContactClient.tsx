...
export default function ContactClient() {
  ...
  const sendEmail = (e: React.FormEvent) => {
    ...
    const phone = fd.get("phone_number")?.toString().trim() || "";
    ...
    emailjs.sendForm(serviceID, templateID, form.current, publicKey).then(
      (result) => {
        ...
        // --- NEW: Also save to Excel/CSV via our local API ---
        if (form.current) {
          const fd = new FormData(form.current);
          const phone = fd.get("phone_number")?.toString() || "";
          if (!/^\+?\d{10,15}$/.test(phone)) {
            setStatus("error");
            setErrorMessage(
              "Invalid phone number. Please enter a valid phone number.",
          ...
          fetch("/api/submit-enrollment", {
            ...
            body: JSON.stringify({
              ...
              phoneNumber: phone,
              whichLevel: `Contact: ${fd.get("message")?.toString().substring(0, 50)}...`,
            }),
      ...
      <section className="px-6 py-20">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            {/* Contact Form */}
            <div>
              <h2 className="text-2xl font-bold text-[#11113F] mb-6">
                Send us a message
              </h2>
              <form ref={form} onSubmit={sendEmail} className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-[#11113F]/60 mb-1.5">
                      First Name
                    </label>
                    <input
                      name="first_name"
            ...
            <div>
              ...
              <div className="space-y-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#11113F]/50">
              ...
              <div className="mt-10 p-6  rounded-xl">
                <p className="text-sm font-semibold text-[#11113F] mb-3">
                  Follow us
                </p>
                <div className="flex gap-3">
                  <a
                    href="https://www.linkedin.com/company/data-craft-academy/posts/?feedView=all"
                    className="w-10 h-10 bg-[#11113F] hover:bg-[#5A4DE5] rounded-full flex items-center justify-center transition-colors"
                  >
                    <svg
